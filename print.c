...
void print_side(byte* cube, Side side, bool padding, bool num) {
    for (int i=0; i<9; i++) {
...
void print_cube(byte* cube, bool num) {
...
void print_cli(byte* cube) {
  byte done = 1;
  for (int i=0;i<54;i++) {
    done = (done && cube[i]==(int) i/9);
    printf("%c", tblocks[cube[i]]);
  } 
  printf(" %d\n", done);
}
